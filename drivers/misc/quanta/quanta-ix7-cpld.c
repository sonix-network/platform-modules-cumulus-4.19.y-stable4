/*
 * CPLD sysfs driver for quanta_ix7.
 *
...
#define DRIVER_NAME "quanta_ix7_cpld"
...
enum {
	_INV_IDX = -1,
	_QSFP28_LPMODE_IDX = 0,
	_QSFP28_PRESENT_IDX,
	_QSFP28_INTERRUPT_IDX,
	_QSFP28_RESET_IDX,
};

struct quanta_ix7_bde_platform_data {
	int idx;
};
...
struct port_status {
	char name[QUANTA_IX7_CPLD_STRING_NAME_SIZE];
	u8 index;
	u8 active_low;
	u8 num_bits;
	u8 num_reg;
	u8 reg[QUANTA_IX7_REG_MAX];
};

static struct port_status qsfp28_status[] = {
	{
		.name = "reset",
		.index = _QSFP28_RESET_IDX,
		.active_low = 1,
		.num_bits = 16,
		.num_reg = 4,
		.reg = { IX7_IO_QSFP28_1_4_INFO_REG,
			 IX7_IO_QSFP28_5_8_INFO_REG,
			 IX7_IO_QSFP28_9_12_INFO_REG,
			 IX7_IO_QSFP28_13_16_INFO_REG},
	},
	{
		.name = "interrupt",
		.index = _QSFP28_INTERRUPT_IDX,
		.active_low = 1,
		.num_bits = 16,
		.num_reg = 4,
		.reg = { IX7_IO_QSFP28_1_4_INFO_REG,
			 ...
			 char *buf)
{
	...
	num_platform_status = ARRAY_SIZE(qsfp28_status);
...
static SENSOR_DEVICE_ATTR_RW(qsfp28_1_16_reset,      port_show, port_store, 0);
static SENSOR_DEVICE_ATTR_RO(qsfp28_1_16_interrupt,  port_show,             1);
static SENSOR_DEVICE_ATTR_RO(qsfp28_1_16_present,    port_show,             2);
static SENSOR_DEVICE_ATTR_RW(qsfp28_1_16_lpmode,     port_show, port_store, 3);
static SENSOR_DEVICE_ATTR_RW(qsfp28_17_32_reset,     port_show, port_store, 0);
static SENSOR_DEVICE_ATTR_RO(qsfp28_17_32_interrupt, port_show,             1);
static SENSOR_DEVICE_ATTR_RO(qsfp28_17_32_present,   port_show,             2);
static SENSOR_DEVICE_ATTR_RW(qsfp28_17_32_lpmode,    port_show, port_store, 3);
			...
			char *buf)
{
	...
	ret = cpld_port_reg_byte_read(client, IX7_LED_DECODER_REG, &val);
			 ...
			 const char *buf, size_t count)
{
	...
	ret = cpld_port_reg_byte_write(client, IX7_LED_DECODER_REG, val);
	if (ret < 0)
		return ret;
...
static SENSOR_DEVICE_ATTR_RW(port_1_16_led_decoder,  led_show, led_store, 0);
static SENSOR_DEVICE_ATTR_RW(port_17_32_led_decoder, led_show, led_store, 1);
...
static struct attribute *qsfp28_1_16_attrs[] = {
	&sensor_dev_attr_qsfp28_1_16_reset.dev_attr.attr,
	&sensor_dev_attr_qsfp28_1_16_interrupt.dev_attr.attr,
	&sensor_dev_attr_qsfp28_1_16_present.dev_attr.attr,
	&sensor_dev_attr_qsfp28_1_16_lpmode.dev_attr.attr,
	NULL,
};

static struct attribute *qsfp28_17_32_attrs[] = {
	&sensor_dev_attr_qsfp28_17_32_reset.dev_attr.attr,
	&sensor_dev_attr_qsfp28_17_32_interrupt.dev_attr.attr,
	&sensor_dev_attr_qsfp28_17_32_present.dev_attr.attr,
	&sensor_dev_attr_qsfp28_17_32_lpmode.dev_attr.attr,
	NULL,
};

static struct attribute *port_1_16_led_decoder_attrs[] = {
	&sensor_dev_attr_port_1_16_led_decoder.dev_attr.attr,
	NULL,
};

static struct attribute *port_17_32_led_decoder_attrs[] = {
	&sensor_dev_attr_port_17_32_led_decoder.dev_attr.attr,
	NULL,
};

static struct attribute_group qsfp28_1_16_attr_group = {
	.attrs = qsfp28_1_16_attrs,
};

static struct attribute_group qsfp28_17_32_attr_group = {
	.attrs = qsfp28_17_32_attrs,
};

static struct attribute_group port_1_16_led_decoder_attr_group = {
	.attrs = port_1_16_led_decoder_attrs,
};

static struct attribute_group port_17_32_led_decoder_attr_group = {
	.attrs = port_17_32_led_decoder_attrs,
};
...
{
	struct quanta_ix7_bde_platform_data *pdata;
	int retval;
	int cpld_idx;
	 ...
	 */
	switch (cpld_idx) {
	case IX7_LED_QSFP28_1_16_CPLD_ID:
		retval = sysfs_create_group(kobj,
					    &port_1_16_led_decoder_attr_group);
		break;
	case IX7_IO_QSFP28_1_16_CPLD_ID:
		retval = sysfs_create_group(kobj,
					    &qsfp28_1_16_attr_group);
		break;
	case IX7_IO_QSFP28_17_32_CPLD_ID:
		retval = sysfs_create_group(kobj,
					    &qsfp28_17_32_attr_group);
		break;
	case IX7_LED_QSFP28_17_32_CPLD_ID:
		retval = sysfs_create_group(kobj,
					    &port_17_32_led_decoder_attr_group);
		break;
...
static int cpld_remove(struct i2c_client *client)
{
	...
	sysfs_remove_group(kobj, &qsfp28_1_16_attr_group);
	sysfs_remove_group(kobj, &qsfp28_17_32_attr_group);
	sysfs_remove_group(kobj, &port_1_16_led_decoder_attr_group);
	sysfs_remove_group(kobj, &port_17_32_led_decoder_attr_group);
...
static int __init quanta_ix7_cpld_init(void)
{
	int rv;
...
static void __exit quanta_ix7_cpld_exit(void)
{
...
MODULE_DESCRIPTION("CPLD driver for Quanta IX7 Platform");
MODULE_LICENSE("GPL");
MODULE_VERSION(DRIVER_VERSION);

module_init(quanta_ix7_cpld_init);
module_exit(quanta_ix7_cpld_exit);
