...
/*
 *  dellemc_n32xx-cpu-cpld.c - Dell EMC N32xx CPU CPLD support.
 *
...
cpld_rg_ro(major_revision, DELL_N32XX_CPU_CPLD_REV_REG1_REG, NULL, 0);
cpld_rg_ro(minor_revision, DELL_N32XX_CPU_CPLD_REV_REG0_REG, NULL, 0);
cpld_rg_rw(scratch, DELL_N32XX_CPU_CPLD_GPR_REG, NULL, 0);
cpld_bf_ro(board_revision, DELL_N32XX_CPU_BRD_REV_TYPE_REG,
	   DELL_N32XX_CPU_BRD_REV_TYPE_BRD_REV, NULL, 0);
cpld_bf_ro(board_type, DELL_N32XX_CPU_BRD_REV_TYPE_REG,
	   DELL_N32XX_CPU_BRD_REV_TYPE_BRD_TYPE, NULL, 0);
cpld_bt_rw(primary_bios_wp, DELL_N32XX_CPU_BIOS_WP_REG,
	   DELL_N32XX_CPU_BIOS_WP_SPI_BIOS_WP, NULL, 0);
cpld_bt_rw(backup_bios_wp, DELL_N32XX_CPU_BIOS_WP_REG,
	   DELL_N32XX_CPU_BIOS_WP_SPI_BAK_BIOS_WP, NULL, 0);
cpld_bf_rw(wd_timer, DELL_N32XX_CPU_SYS_WD_REG, DELL_N32XX_CPU_SYS_WD_WD_TIMER,
	   wd_timer_values, 0);
cpld_bt_rw(wd_enable, DELL_N32XX_CPU_SYS_WD_REG, DELL_N32XX_CPU_SYS_WD_WD_EN,
	   NULL, 0);
cpld_bt_rw(wd_punch, DELL_N32XX_CPU_SYS_WD_REG, DELL_N32XX_CPU_SYS_WD_WD_PUNCH,
	   NULL, BF_COMPLEMENT);
cpld_rg_rw(reboot_cause, DELL_N32XX_CPU_REBOOT_CAUSE_REG, NULL, 0);
cpld_rg_ro(cpu_error_state, DELL_N32XX_CPU_ERR_STS_REG, NULL, 0);
cpld_bf_ro(tpm_id, DELL_N32XX_CPU_MISC_STS1_REG,
	   DELL_N32XX_CPU_MISC_STS1_TPM_ID, NULL, 0);
...
static int cpld_probe(struct i2c_client *client)
{
	...
	ret = i2c_smbus_read_byte_data(client,
				       DELL_N32XX_CPU_CPLD_REV_REG1_REG);
	if (ret < 0) {
		dev_err(&client->dev,
	...
	major_rev = ret;
	ret = i2c_smbus_read_byte_data(client,
				       DELL_N32XX_CPU_CPLD_REV_REG0_REG);
	if (ret < 0) {
		dev_err(&client->dev,
	...
	dev_info(&client->dev,
		 "device created, CPU CPLD major rev %d, minor rev %d\n",
		 major_rev & 0xff,
		 minor_rev & 0xff);
	return 0;
...
MODULE_DESCRIPTION("Dell EMC N32xx CPU CPLD support");
