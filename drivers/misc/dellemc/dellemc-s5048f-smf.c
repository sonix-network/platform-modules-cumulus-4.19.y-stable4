...
/*
 * Smartfusion driver for Dell EMC S5048F
 *
...
#define DRIVER_NAME    S5048F_SMF_NAME
#define DRIVER_VERSION "1.1"
#define IO_BASE        SMF_IO_BASE
#define IO_SIZE        SMF_IO_SIZE
...
{
	return ioread8(smf_regs + reg - IO_BASE);
}

static inline void smf_wr(u32 reg, uint8_t data)
{
	iowrite8(data, smf_regs + reg - IO_BASE);
}
...
static SENSOR_DEVICE_ATTR_RO(smf_version,       smf_show, SMF_VER);
...
static SENSOR_DEVICE_ATTR_RO(protocol_version, mb_show,	SMF_PROTOCOL_VER);
static SENSOR_DEVICE_ATTR_RO(mss_version,      mb_show,	SMF_MSS_VER);
...
static SENSOR_DEVICE_ATTR_RO(temp5_input, mb_temp_show,
			     SMF_TEMP_SENSOR_BASE + 0x08);
static SENSOR_DEVICE_ATTR_RO(temp6_input, mb_temp_show,
			     SMF_TEMP_SENSOR_BASE + 0x0a);
static SENSOR_DEVICE_ATTR_RO(temp7_input, mb_temp_show,
			     SMF_TEMP_SENSOR_BASE + 0x0c);
static SENSOR_DEVICE_ATTR_RO(temp8_input, mb_temp_show,
			     SMF_TEMP_SENSOR_BASE + 0x0e);
static SENSOR_DEVICE_ATTR_RO(temp9_input, mb_temp_show,
			     SMF_TEMP_SENSOR_BASE + 0x10);
static SENSOR_DEVICE_ATTR_RO(temp10_input, mb_temp_show,
			     SMF_TEMP_SENSOR_BASE + 0x12);
...
static SENSOR_DEVICE_ATTR_RO(temp10_fault, mb_temp_fault_show,
			     SMF_TEMP_STATUS_BASE + 0x09);
...
static struct attribute *smf_attrs[] = {
	...
	&sensor_dev_attr_protocol_version.dev_attr.attr,
	&sensor_dev_attr_mss_version.dev_attr.attr,
...
static int smf_probe(struct platform_device *dev)
{
	...
	smf_regs = ioport_map(IO_BASE, IO_SIZE);
...
static int __init smf_init(void)
{
	...
	if (!smf_device) {
		pr_err(DRIVER_NAME ": failed to allocate platform device\n");
		rv = -ENOMEM;
		goto err_unregister;
	}
	...
	if (rv) {
		pr_err(DRIVER_NAME ": failed to add platform device\n");
		goto err_dealloc;
	}
	return 0;
...
MODULE_DESCRIPTION("Smartfusion Driver for Dell EMC S5048F");
MODULE_LICENSE("GPL");
