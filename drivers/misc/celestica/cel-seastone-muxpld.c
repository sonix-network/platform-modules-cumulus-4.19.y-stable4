/*
 * cel_seastone_cpld_muxpld.c - Celestica Seastone Platform Support
 *
...
#define DRIVER_NAME	"cel_seastone_muxpld"
#define DRIVER_VERSION	"1.0"

static struct platform_driver cel_seastone_cpld_mux_driver;

static struct cel_xp_bus_mux_info sea_bus_mux_info[] = {
	{
...
struct cel_sea_mux_data_struc {
	struct platform_device *bus_devices[ARRAY_SIZE(sea_bus_mux_info)];
	struct platform_device *mux_devices[ARRAY_SIZE(sea_bus_mux_info)];
	struct i2c_client *i2c_clients[CEL_SEASTONE_NUM_PORTS];
	struct i2c_board_info *board_infos[CEL_SEASTONE_NUM_PORTS];
};

static struct cel_sea_mux_data_struc cel_sea_mux_data;
...
static void free_rxp_data(void)
{
	...
	for (i = 0; i < CEL_SEASTONE_NUM_PORTS; i++) {
		if (cel_sea_mux_data.i2c_clients[i]) {
			i2c_unregister_device(cel_sea_mux_data.i2c_clients[i]);
		}
		if (cel_sea_mux_data.board_infos[i]) {
			kfree(cel_sea_mux_data.board_infos[i]);
		}
	}

	for (i = 0; i < ARRAY_SIZE(sea_bus_mux_info); i++) {
		if (cel_sea_mux_data.mux_devices[i]) {
			platform_device_del(cel_sea_mux_data.mux_devices[i]);
		}
	}
}
...
static int cel_seastone_cpld_mux_probe(struct platform_device *dev)
{
	return 0;
}

static int cel_seastone_cpld_mux_remove(struct platform_device *dev)
{
	return 0;
}
...
int create_i2c_bus(int index, int bus, u32 io_base)
{
	...
	cel_sea_mux_data.bus_devices[index] = device;
...
int create_i2c_mux(int index, int bus, int first_port_num, int num_ports)
{
	...
	cel_sea_mux_data.mux_devices[index] = device;
	...
	for (i = 0; i < num_ports; i++) {
		...
		cel_sea_mux_data.i2c_clients[port_num - 1] = client;
	}
...
static int __init cel_seastone_cpld_mux_init(void)
{
	int ret;
	int i;

	ret = platform_driver_register(&cel_seastone_cpld_mux_driver);
	if (ret) {
		pr_err("could not register seastone_cpld_mux_driver\n");
		goto err_exit;
	}
	 ...
	 */
	for (i = 0; i < ARRAY_SIZE(sea_bus_mux_info); i++) {
		ret = create_i2c_bus(i, sea_bus_mux_info[i].bus, sea_bus_mux_info[i].io_base);
		if (ret) {
			goto err_exit2;
		}
	}
	 ...
	 */
	for (i = 0; i < ARRAY_SIZE(sea_bus_mux_info); i++) {
		ret = create_i2c_mux(i, sea_bus_mux_info[i].bus,
		                     sea_bus_mux_info[i].first_port_num,
		                     sea_bus_mux_info[i].num_ports);
		if (ret) {
			goto err_exit2;
		}
	}
...
err_exit2:
	free_rxp_data();
	platform_driver_unregister(&cel_seastone_cpld_mux_driver);

err_exit:
	return ret;
}

static void __exit cel_seastone_cpld_mux_exit(void)
{
	free_rxp_data();
	platform_driver_unregister(&cel_seastone_cpld_mux_driver);
...
static struct platform_driver cel_seastone_cpld_mux_driver = {
	...
	.probe = cel_seastone_cpld_mux_probe,
	.remove = cel_seastone_cpld_mux_remove,
};


module_init(cel_seastone_cpld_mux_init);
module_exit(cel_seastone_cpld_mux_exit);

MODULE_AUTHOR("Alan Liebthal (alanl@cumulusnetworks.com)");
MODULE_DESCRIPTION("Celestica Seastone CPLD mux Support");
