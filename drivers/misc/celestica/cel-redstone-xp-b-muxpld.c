/*
 * cel_redstone_cpld_muxpld.c - Celestica Redstone XP-B Platform Support
 *
...
#define DRIVER_NAME	"cel_red_xpb_muxpld"
#define DRIVER_VERSION	"1.0"

static struct platform_driver cel_red_xpb_cpld_mux_driver;

struct cel_red_xpb_bus_mux_info {
	int bus;
	u32 io_base;
	int first_port_num;
	int num_ports;
...
static struct cel_red_xpb_bus_mux_info red_xp_bus_mux_info[] = {
	{
	...
	},
	{
		...
		.io_base = CPLD4_REG_I2C_FREQ_DIV,
		.first_port_num = 49,
		.num_ports = 6,
	},
	{
		.bus = CL_I2C_CPLD_MUX_4,
		.io_base = CPLD5_REG_I2C_FREQ_DIV,
		.first_port_num = 37,
		.num_ports = 12,
	},
};

struct redxp_mux_data_struct {
	...
	struct i2c_client *i2c_clients[CEL_REDSTONE_XP_B_NUM_PORTS];
	struct i2c_board_info *board_infos[CEL_REDSTONE_XP_B_NUM_PORTS];
};

static struct redxp_mux_data_struct redxp_mux_data;
...
static void free_rxp_data(void)
{
	...
	for (i = 0; i < CL_I2C_CPLD_MUX_FIRST_PORT; i++) {
		if (redxp_mux_data.i2c_clients[i])
			i2c_unregister_device(redxp_mux_data.i2c_clients[i]);
		kfree(redxp_mux_data.board_infos[i]);
	}

	for (i = 0; i < ARRAY_SIZE(red_xp_bus_mux_info); i++) {
		if (redxp_mux_data.mux_devices[i])
			platform_device_del(redxp_mux_data.mux_devices[i]);
		if (redxp_mux_data.bus_devices[i])
			platform_device_del(redxp_mux_data.bus_devices[i]);
	}
}
...
int create_i2c_bus(int index, int bus, u32 io_base)
{
	...
	redxp_mux_data.bus_devices[index] = device;
...
int create_i2c_mux(int index, int bus, int first_port_num, int num_ports)
{
	...
	redxp_mux_data.mux_devices[index] = device;
	...
	for (i = 0; i < num_ports; i++) {
		...
		redxp_mux_data.i2c_clients[port_num] = client;
	}
...
static int cel_red_xpb_cpld_mux_probe(struct platform_device *dev)
{
	int i;
	int ret = 0;
	 ...
	 */
	for (i = 0; i < ARRAY_SIZE(red_xp_bus_mux_info); i++) {
		ret = create_i2c_bus(i, red_xp_bus_mux_info[i].bus,
				     red_xp_bus_mux_info[i].io_base);
		if (ret)
			goto err_exit;
	}
	/*
...
static int cel_red_xpb_cpld_mux_remove(struct platform_device *dev)
{
	free_rxp_data();
...
static const struct platform_device_id cel_red_xpb_platform_id[] = {
	{ DRIVER_NAME, 0 },
	{ }
};
MODULE_DEVICE_TABLE(platform, cel_red_xpb_platform_id);
...
static struct platform_driver cel_red_xpb_cpld_mux_driver = {
	...
	.probe = cel_red_xpb_cpld_mux_probe,
	.remove = cel_red_xpb_cpld_mux_remove,
	.id_table = cel_red_xpb_platform_id
};

static struct platform_device *plat_device = NULL;

static int __init cel_red_xpb_cpld_mux_init(void)
{
	int ret = 0;
	...
	if (!driver_find(cel_red_xpb_cpld_mux_driver.driver.name,
			 &platform_bus_type)) {
		ret = platform_driver_register(&cel_red_xpb_cpld_mux_driver);
		if (ret) {
			pr_err(DRIVER_NAME ": %s driver registration failed."
			       "(%d)\n", 
			       cel_red_xpb_cpld_mux_driver.driver.name, ret);
			return ret;
		}
	}
	...
	if (plat_device == NULL) {
		...
		if (IS_ERR(plat_device)) {
			...
			pr_err(DRIVER_NAME": Platform device registration"
			       "failed. (%d)\n", ret);
			platform_driver_unregister(&cel_red_xpb_cpld_mux_driver);
			return ret;
		}
	}
...
static void __exit cel_red_xpb_cpld_mux_exit(void)
{
...
module_init(cel_red_xpb_cpld_mux_init);
module_exit(cel_red_xpb_cpld_mux_exit);
