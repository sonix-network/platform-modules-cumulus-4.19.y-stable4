/*
 * cel_redstone_cpld_muxpld.c - Celestica Redstone XP Platform Support
 *
...
#define DRIVER_NAME	"cel_redstone_xp_cpld_muxpld"
#define DRIVER_VERSION	"1.1"

static struct platform_driver cel_redstone_xp_cpld_mux_driver;

struct cel_redstone_xp_bus_mux_info {
	int bus;
	u32 io_base;
	int first_port_num;
	int num_ports;
...
static struct cel_redstone_xp_bus_mux_info red_xp_bus_mux_info[] = {
	{
	...
	{
		.bus = RXP_I2C_CPLD_MUX_3,
		.io_base = CPLD4_REG_I2C_PORT_ID_OFFSET,
		.first_port_num = 49,
		.num_ports = 6,
	},
	{
		.bus = RXP_I2C_CPLD_MUX_4,
		.io_base = CPLD5_REG_I2C_PORT_ID_OFFSET,
		.first_port_num = 37,
		.num_ports = 12,
	},
};

struct cel_redxp_mux_data_struc {
	struct platform_device *bus_devices[ARRAY_SIZE(red_xp_bus_mux_info)];
	struct platform_device *mux_devices[ARRAY_SIZE(red_xp_bus_mux_info)];
	struct i2c_client *i2c_clients[CEL_REDSTONE_XP_NUM_PORTS];
	struct i2c_board_info *board_infos[CEL_REDSTONE_XP_NUM_PORTS];
};

static struct cel_redxp_mux_data_struc cel_redxp_mux_data;
...
static void free_rxp_data(void)
{
	...
	for (i = 0; i < CEL_REDSTONE_XP_NUM_PORTS; i++) {
		if (cel_redxp_mux_data.i2c_clients[i]) {
			i2c_unregister_device(cel_redxp_mux_data.i2c_clients[i]);
		}
		if (cel_redxp_mux_data.board_infos[i]) {
			kfree(cel_redxp_mux_data.board_infos[i]);
		}
	}

	for (i = 0; i < ARRAY_SIZE(red_xp_bus_mux_info); i++) {
		if (cel_redxp_mux_data.mux_devices[i]) {
			platform_device_del(cel_redxp_mux_data.mux_devices[i]);
		}
		if (cel_redxp_mux_data.bus_devices[i]) {
//			platform_device_del(cel_redxp_mux_data.bus_devices[i]);  aplx
		}
	}
}

static bool cel_red_xp_qsfp_port(int port) {
	if (port >= 49)
		return true;
	else
		return false;
...
static int cel_redstone_xp_cpld_mux_probe(struct platform_device *dev)
{
	return 0;
}

static int cel_redstone_xp_cpld_mux_remove(struct platform_device *dev)
{
	return 0;
}
...
int create_i2c_bus(int index, int bus, u32 io_base)
{
	...
	cel_redxp_mux_data.bus_devices[index] = device;
...
int create_i2c_mux(int index, int bus, int first_port_num, int num_ports)
{
	...
	cel_redxp_mux_data.mux_devices[index] = device;
	...
	for (i = 0; i < num_ports; i++) {
		...
		if (cel_red_xp_qsfp_port(port_num)) {
		...
		cel_redxp_mux_data.board_infos[port_num - 1] = board_info;
		...
		cel_redxp_mux_data.i2c_clients[port_num - 1] = client;
	}
...
static int __init cel_redstone_xp_cpld_mux_init(void)
{
	int ret;
	int i;

	ret = platform_driver_register(&cel_redstone_xp_cpld_mux_driver);
	if (ret) {
		pr_err("could not register cel_xp_cpld_mux_driver\n");
		goto err_exit;
	}
	 ...
	 */
	for (i = 0; i < ARRAY_SIZE(red_xp_bus_mux_info); i++) {
		ret = create_i2c_bus(i, red_xp_bus_mux_info[i].bus, red_xp_bus_mux_info[i].io_base);
		if (ret) {
			goto err_exit2;
		}
	}
	 ...
	 */
	for (i = 0; i < ARRAY_SIZE(red_xp_bus_mux_info); i++) {
		ret = create_i2c_mux(i, red_xp_bus_mux_info[i].bus,
		                     red_xp_bus_mux_info[i].first_port_num,
		                     red_xp_bus_mux_info[i].num_ports);
		if (ret) {
			goto err_exit2;
		}
	}
...
err_exit2:
	free_rxp_data();
	platform_driver_unregister(&cel_redstone_xp_cpld_mux_driver);

err_exit:
	return ret;
}

static void __exit cel_redstone_xp_cpld_mux_exit(void)
{
	free_rxp_data();
	platform_driver_unregister(&cel_redstone_xp_cpld_mux_driver);
...
static struct platform_driver cel_redstone_xp_cpld_mux_driver = {
	...
	.probe = cel_redstone_xp_cpld_mux_probe,
	.remove = cel_redstone_xp_cpld_mux_remove,
};


module_init(cel_redstone_xp_cpld_mux_init);
module_exit(cel_redstone_xp_cpld_mux_exit);

MODULE_AUTHOR("Alan Liebthal (alanl@cumulusnetworks.com)");
MODULE_DESCRIPTION("Celestica Redstone XP CPLD mux Support");
