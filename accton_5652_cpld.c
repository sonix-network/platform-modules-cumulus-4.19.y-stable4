/*
 * CPLD sysfs driver for accton_5652.
 *
...
#include "accton_5652_cpld.h"
#include "platform_defs.h"

static const char driver_name[] = "accton_5652_cpld";
...
static uint8_t __iomem* accton_5652_cpld_regs;
static DEFINE_MUTEX(accton_5652_cpld_mutex);
...
{
	return readb(accton_5652_cpld_regs + reg);
}

static inline void cpld_wr(uint32_t reg, uint8_t data)
{
	writeb(data, accton_5652_cpld_regs + reg);
}

#define ACCTON_5652_CPLD_STRING_NAME_SIZE 30
 ...
 */
struct cpld_status {
	char name[ACCTON_5652_CPLD_STRING_NAME_SIZE];
	uint8_t good_mask;  // set bits for "good behaviour"
	uint8_t bad_mask;   // set bits for "bad behaviour"
	char msg_good[ACCTON_5652_CPLD_STRING_NAME_SIZE]; // positive string message
	char msg_bad[ACCTON_5652_CPLD_STRING_NAME_SIZE]; // negative string message
};
...
static struct led cpld_leds[] = {
	{
		...
		.colors = {
			...
			{ PLATFORM_LED_HW_CTRL, CPLD_SYS_LED_PS1_HW_CTRL},
		},
	},
	{
		.name = "led_psu2",
		...
		.colors = {
			...
			{ PLATFORM_LED_HW_CTRL, CPLD_SYS_LED_PS2_HW_CTRL},
		},
	},
	{
		.name = "led_diag",
		...
		.n_colors = 3,
		.colors = {
			{ PLATFORM_LED_YELLOW_BLINKING, CPLD_SYS_LED_DIAG_YELLOW_BLINK},
	...
	},
	{
		...
		.n_colors = 2,
		.colors = {
			{ PLATFORM_LED_GREEN_BLINKING, CPLD_SYS_LED_LOCATOR_GREEN_BLINK},
			 ...
			 const char * buf, size_t count)
{
	...
	mutex_lock(&accton_5652_cpld_mutex);
	...
	mutex_unlock(&accton_5652_cpld_mutex);
...
/*
 * Flash Swizzle feature (Boot Flash Control)
 */

struct flash_swizzle {
	char name[ACCTON_5652_CPLD_STRING_NAME_SIZE];
	uint8_t bit;
};

struct flash_swizzle flash_swizzle_bits[] = {
	{
		.name = "flash_swizzle_active",
		.bit  = 0,
	},
	{
		.name = "flash_swizzle_enable",
		.bit  = 1,
	},
};

static int n_flash_swizzle_bits = ARRAY_SIZE(flash_swizzle_bits);

static ssize_t flash_swizzle_show(struct device * dev,
			       struct device_attribute * dattr,
			       char * buf)
{
	uint8_t tmp;
	int i;
	struct flash_swizzle* target = NULL;

	/* find the target bit */
	for (i = 0; i < n_flash_swizzle_bits; i++) {
		if (strcmp(dattr->attr.name, flash_swizzle_bits[i].name) == 0) {
			target = &flash_swizzle_bits[i];
			break;
		}
	}
	if (target == NULL) {
	...
	/* read the register */
	tmp = cpld_rd(CPLD_REG_FLASH_SWIZZLE);

	return sprintf(buf, "%d\n", (tmp & (1 << target->bit)) ? 1 : 0);

}

static ssize_t flash_swizzle_store(struct device * dev,
				struct device_attribute * dattr,
				const char * buf, size_t count)
{
	uint8_t tmp;
	...
	cpld_wr(CPLD_REG_FLASH_SWIZZLE, tmp);
...
static SYSFS_ATTR_RW(flash_swizzle_active,    flash_swizzle_show, flash_swizzle_store);
static SYSFS_ATTR_RW(flash_swizzle_enable,    flash_swizzle_show, flash_swizzle_store);
...
\n\
  Locator LED color.\n\
  The following values are possible: yellow_blinking, off\n\
\n\
led_psu1\n\
led_psu2\n\
\n\
...
\n\
  PSU LED colors.\n\
  The following values are possible: green, yellow, hw_ctrl\n\
\n\
  hw_ctrl -- Let the HW control the LED color.\n\
\n\
psu_pwr1_all_ok\n\
psu_pwr1_dc_ok\n\
psu_pwr1_present\n\
...
\n\
    psu_pwr1_all_ok =\n\
      psu_pwr1_present && psu_pwr1_dc_ok;\n\
\n\
\n\
system_all_ok\n\
system_fan_ok\n\
...
\n\
flash_swizzle_active\n\
\n\
  Read-Write:\n\
\n\
  set/get the flash swizzle active bit in the CPLD, which controls\n\
  which half of the boot flash is active.\n\
\n\
  0 -- The bottom half of the boot flash is active.\n\
  1 -- The top half of the boot flash is active.\n\
\n\
flash_swizzle_enable\n\
\n\
  Read-Write:\n\
\n\
  set/get the flash swizzle enable bit in the CPLD.\n\
\n\
  0 -- Normal boot.  The system booted using the bottom half of the\n\
       boot flash.\n\
  1 -- Alternate boot.  The system booted using the top half of the\n\
       boot flash.\n\
\n\
  The alternate boot happens when the system is unable to boot from\n\
  the boot half with in a fixed amount of time.\n\
\n\
  After the system has booted, writing this bit does not make much\n\
  sense.\n\
\n\
watch_dog_enable\n\
\n\
...
static struct attribute *accton_5652_cpld_attrs[] = {
	...
	&dev_attr_flash_swizzle_active.attr,
	&dev_attr_flash_swizzle_enable.attr,
...
static struct attribute_group accton_5652_cpld_attr_group = {
	.attrs = accton_5652_cpld_attrs,
};
...
static int accton_5652_cpld_setup(void)
{
	// Put some interesting, one-time initializations here.
	return 0;
}

static int accton_5652_cpld_probe(struct platform_device * ofdev)
{
	int retval = 0;
	...
	accton_5652_cpld_regs = of_iomap(np,0);
	if (!accton_5652_cpld_regs) {
		return -EIO;
	}

	retval = sysfs_create_group(kobj, &accton_5652_cpld_attr_group);
	if (retval) {
		return retval;
	}

	if (accton_5652_cpld_setup()) {
	     return -EIO;
	}

	dev_info(&ofdev->dev, "probed & iomapped @ 0x%p\n", accton_5652_cpld_setup);

	return 0;
}

static int accton_5652_cpld_remove(struct platform_device * ofdev)
{
	struct kobject * kobj = &ofdev->dev.kobj;

	/* don't iounmap(regs)... the platform driver uses it for reset	*/
	sysfs_remove_group(kobj, &accton_5652_cpld_attr_group);

	dev_info(&ofdev->dev, "removed\n");
	return 0;
}

static struct of_device_id accton_5652_cpld_ids[] = {
	{
...
static struct platform_driver accton_5652_cpld_driver = {
	.probe = accton_5652_cpld_probe,
	.remove = accton_5652_cpld_remove,
	.driver = {
		.name  = driver_name,
		.owner = THIS_MODULE,
		.of_match_table = accton_5652_cpld_ids,
	},
};
...
static int __init accton_5652_cpld_init(void)
{
	int rv;

	rv = platform_driver_register(&accton_5652_cpld_driver);
	if (rv) {
		printk(KERN_ERR
...
static void __exit accton_5652_cpld_exit(void)
{
	return platform_driver_unregister(&accton_5652_cpld_driver);
}

MODULE_AUTHOR("Curt Brune <curt@cumulusnetworks.com>");
MODULE_DESCRIPTION("CPLD driver for Accton Technology Corporation, ES5652BT1");
MODULE_LICENSE("GPL");
MODULE_VERSION(DRIVER_VERSION);

module_init(accton_5652_cpld_init);
module_exit(accton_5652_cpld_exit);
